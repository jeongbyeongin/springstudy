...
public class FirstController {
	...
	@GetMapping(value="/first/ajax1", produces="application/json")	// produces : 응답데이터의 MIME TYPE,  value 속성만 있으면 value를 제외해도된다.
	public Person ajax(HttpServletRequest request,
					   HttpServletResponse response) {				
		return firstService.execute1(request, response);				// "Jackson 라이브러리"가 반환값 Person 객체를 자동으로 JSON 데이터로 변환한다.
																	// 여기서는 jsp의 이름이였는데  ResponseBody를 해줘서 이름이 아닌게됬다.
	}
	...
	public Map<String, Object> ajax3(@RequestParam("name") String name, @RequestParam("age") int age){
		return firstService.execute3(name, age);
	}
