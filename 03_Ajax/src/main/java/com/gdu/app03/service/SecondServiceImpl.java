...
public class SecondServiceImpl implements ISecondService {
	...
	public BmiVO execute1(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			return new BmiVO(weight, height, bmi, obesity);		// $.ajax의 error로 넘기는 값
			
		} catch(Exception e) {		// parse에서 발생하는 이셉션이라서 NumberFormatException이 정확하다.
	 ...
	 */
	/*
		ResponseEntity<응답데이터타입> 클래스
		1. Ajax 응답 데이터를 생성하는 클래스 
		2. 생성자 중 하나의 사용법
			public ResponseEntity(@Nullable T body, @Nullable MultiValueMap<String, String> headers, HttpStatus status)
			1) @Nullable T body 					 : 실제로 응답 할 데이터
			2) MultiValueMap<String, String> headers : 응답 헤더(대표적인 응답 헤더 : Content - Type)
			3) HttpStatus status					 : Spring에서 사용하는 응답 코드(200, 400, 500 등)
	*/
	@Override
	...
	public ResponseEntity<Map<String, Object>> execute2(BmiVO bmiVO) {
		...
		double bmi = weight / (height * height / 10000);	// bmi = 몸무게 / 키(m)*키(m)
		String obesity = null;
		if(bmi < 18.5) {
			obesity = "저체중";
		} else if(bmi < 24.9) {
		...
		// 응답 헤더(Content-Type)	// HttpHeaders 의 인터페이스 타입이 MultiValueMap이다.
