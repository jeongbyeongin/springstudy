...
/*
	...
	2. Bean 등록 방식 3가지 
		...
		1) root-context.xml에 <bean> 태그를 추가한 경우
			@ContextConfiguration(locations={"file:src/main/wepapp/WEP-INF/spring/root-context.xml"})	// 참고할 파일의 경로가 들어간다.
			
		2) com.gdu.app04.config.AppConfig.java에 @Configuration + @Bean 을 작성한 경우
			@ContextConfiguration(classes={AppConfig.class})
		
		3) @Component(@Controller, @Service, @Repository)
			@ContextConfiguration(locations={"file:src/main/wepapp/WEP-INF/spring/appServlet/servlet-context.xml"})
 */
...
public class BoardUnitTest {
	...
	// BoardUnitTest 클래스 실행할 때 Logger를 동작시킨다.
	...
	@Test
	public void a3수정테스트() {
		BoardDTO board = new BoardDTO(1, "[수정]제목", "[수정]내용", null, null, null); // null 값으로 처리된 아이들은 수정할 수 없다.
		assertEquals(1, boardDAO.updateBoard(board));	// boardDAO.updateBoard(board)의 결과가 1이면 테스트 성공! 
	}
...
//		System.out.println("a5 - " + list);	// LOG기능으로 대체할 예정
		LOGGER.info(list.toString());
		assertEquals(1, list.size());	// 목록 개수가 1이면 테스트 성공!@#!@#!@#
	}
	
	@Test
	public void a6삭제테스트() {
		int board_no = 1;	// 삭제할 게시글 번호
		assertEquals(1, boardDAO.deleteBoard(board_no)); // boardDAO.deleteBoard(board_no)의 결과가 1이면 테스트 성공!@$$!
	}
