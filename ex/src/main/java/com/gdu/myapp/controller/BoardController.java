...
public class BoardController {
	...
	*/
	...
	/*
	// <a>, location으로 넘어오는 것은 모두 GetMappin이다.
	...
	*/
	/*
	@GetMapping("/detail.do")
	public void getDetail(@RequestParam(value="boardNo", required=false, defaultValue="0") int boardNo 	// required=false 필수가 아니다. defaultValue="0" 필수가 아니다 없다면 0으로 해라
	...
	*/
	...
	public String getDetail(HttpServletRequest request, Model model) {
		...
		return "detail";						 // detail.jsp로 boardNo와 title을 전달.
	}
	...
	*/
	...
	/*
	@GetMapping("/detail.do")
	public String getDetail(@RequestParam(value="boardNo", required=false, defaultValue="0") int boardNo 	// required=false 필수가 아니다. defaultValue="0" 필수가 아니다 없다면 0으로 해라
					    , @RequestParam(value="title", required=false, defaultValue="빈제목") String title
					    , Model model) {
		model.addAttribute("boardNo", boardNo);
		model.addAttribute("title", title);
	...
	*/
	...
	@GetMapping("/detail.do")
	public String getDetail(BoardDTO board) {	// (커맨드 객체의 특징)파라미터를 객체로 받으면 자동으로 Model에 저장이 된다.!!! 속성명은 boardDTO로 저장이 된다!(클래스를 이용해서 속성을 만든다!!)
		return "detail";
	}
	@PostMapping("/detail.do")
